...
import ExportExcelButton from "../components/exportOrdersToExcel.js";
...
const DemSLBlock = ({ totalQuantities, totalQuantitiesINDON, totalQuantitiesCTYDONG, totalQuantitiesKHODONG, totalQuantitiesDeleted, totalRevenue, currentUser, visibleSections, onClose }) => {
  const SECTIONS = [
    ...
    visibleSections.deleted && {
      key: 'deleted',
      title: 'Sản phẩm đã xóa',
      subtitle: 'Thống kê các đơn đã xóa trong bộ lọc',
      data: totalQuantitiesDeleted,
      accent: '#b91c1c',
      accent2: '#fca5a5',
      icon: '🗑️',
    },
...
const [products2, setProducts] = useState([]);
  ...
  const [totalQuantitiesDeleted, setTotalQuantitiesDeleted] = useState({});
...
const resetPagename =()=>{
  ...
  if (currentUser.position === "kho2") {
    ...
    // Chỉ giữ những đơn đã xóa (cột Ngày xóa DS - orderDate5 - có giá trị)
    roleFilteredOrders = roleFilteredOrders.filter(
      (order) => order.orderDate5 && String(order.orderDate5).trim() !== ""
    );
...
//   }, 0);
  ...
  const handleCalculateTotals = () => {
    ...
    setTotalQuantities(totals);
      ...
      // Trang LastDeleteOrder: filteredOrders đã được lọc chỉ còn các đơn đã xóa (orderDate5 có giá trị)
      // nên thống kê sản phẩm của filteredOrders chính là thống kê sản phẩm đã xóa
      const totalsDeleted = calculateTotalQuantities(filteredOrders);
      setTotalQuantitiesDeleted(totalsDeleted);
  ...
  Đặt STT về 0
          ...
          <Row gutter={[12, 12]} align="middle">
            ...
            <Col xs={24} md={14}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, justifyContent: 'flex-end', flexWrap: 'wrap' }}>
                {/* Đã ẩn nút Đếm SL trên trang LastDeleteOrder - thống kê tự động theo bộ lọc */}
                {/* <Button
                  type="primary"
      ...
      {(currentUser.position_team==="kho"||currentUser.position_team==="mkt" ||currentUser.position ==="leadSALE"||currentUser.position ==="admin"||currentUser.position ==="managerSALE"||  currentUser.name ==="Hoàng Lan Phương"  )&& isdem && (
        <DemSLBlock
          ...
          totalQuantitiesDeleted={totalQuantitiesDeleted}
...
>
  ...
  <Table
    ...
    columns={[
      ...
      { title: 'Sản phẩm', key: 'products', width: 240, render: (_, record) => (
        <span className="customer-history-product">{record.products?.map(p => `${p.product} - SL: ${p.quantity}`).join(', ')}</span>
      )},
      { title: 'Tên Khách', dataIndex: "customerName",
        key: "customerName", width: 150, render: (text) => (
          <span className="customer-history-customer">{text}</span>
        )},
      ...
      : []),
        ...
        { title: 'Doanh số', dataIndex: 'revenue', key: 'revenue', width: 130, render: (text) => (
          <span className="customer-history-revenue">{Number(text || 0).toLocaleString()}</span>
        )},
        { title: 'SĐT', dataIndex: 'phone', key: 'phone', width: 130 },
      { title: 'Ngày đặt', dataIndex: 'orderDate4', key: 'orderDate', width: 150, render: (text, record) => {
        const dateValue = text || record.orderDate;
        if (!dateValue) return "N/A";
        const date = dayjs(dateValue);
        return (
     ...
     
    {
          ...
          render: (text) => {
            if (!text) return ""; // Tránh lỗi nếu note rỗng hoặc null
            const parts = text.split(":");
            return <div className="customer-history-note" style={{ width: 200 }}><h3>{parts.length > 1 ? parts.slice(1).join(":").trim() : text}</h3></div>;
          },
        },{
      title:
          "TT XỬ LÍ",
      ...
      width: 120,
      render: (text) => (
        <Tag className="customer-history-tag">{text}</Tag>
      ),
    },
    {
      title: 
