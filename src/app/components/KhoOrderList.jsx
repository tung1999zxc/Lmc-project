...
.kho-root{background:var(--bg);color:var(--ink);display:flex;overflow:hidden;-webkit-font-smoothing:antialiased;height:100vh;width:100%}
...
/* CONTENT */
.content{flex:1;overflow:auto;padding:14px 18px 80px;display:flex;flex-direction:column;gap:12px}
...
/* TABLE CARD */
.tcard{background:var(--card);border:1px solid var(--line);border-radius:var(--r);overflow:hidden}
...
function mapOrder(o) {
  ...
  const delivered =
    o.deliveryStatus === "GIAO THÀNH CÔNG" || o.istick4 === true;
...
export default function KhoOrderList() {
  ...
  const fetchOrders = useCallback(async () => {
    setLoading(true);
    try {
      const res = await axios.get("/api/orderskho", {
  ...
  async function saveDelivered() {
    // Lọc những đơn đã tick Giao TC trong bảng hiện tại
    const toUpdate = filteredList.filter((o) => deliveredIds.has(o.id) && !o.delivered);
    if (!toUpdate.length) { showToast("Không có đơn nào để lưu"); return; }
    try {
      await axios.post("/api/orders/updateIstickDONE", {
        orders: toUpdate.map(({ id }) => ({ id, istickDONE: true })),
      });
  ...
  // ── Tick tất cả Giao TC ──
  function tickAllDelivered() {
    setDeliveredIds((prev) => {
      const next = new Set(prev);
      filteredList.forEach((o) => next.add(o.id));
      return next;
    });
    showToast(`Đã tích Giao TC ${filteredList.length} đơn`);
  }

  // ── Tick tất cả Đối soát ──
  function tickAllReconcile() {
    setReconciledIds((prev) => {
      const next = new Set(prev);
      filteredList.forEach((o) => next.add(o.id));
      return next;
    });
    showToast(`Đã tích Đối soát ${filteredList.length} đơn`);
  }
  ...
  function bulkDeliver() {
    ...
    setDeliveredIds((prev) => {
      const next = new Set(prev);
      sel.forEach((o) => next.add(o.id));
      return next;
    });
    showToast(`Giao TC ${sel.length} đơn`);
    clearSelection();
  }
  ...
  function bulkReconcile() {
    ...
    showToast(`Đối soát ${sel.length} đơn`);
    clearSelection();
    setTimeout(() => setCurrentView("reconcile"), 600);
  }

  // ── Bulk unreconcile ──
  function bulkUnreconcile() {
    const sel = filteredList.filter((o) => selectedIds.has(o.id));
    setReconciledIds((prev) => {
      const next = new Set(prev);
      sel.forEach((o) => next.delete(o.id));
      return next;
    });
    showToast(`Bỏ đối soát ${sel.length} đơn`);
    clearSelection();
  }
  ...
  function exportExcel() {
    ...
    const rows = filteredList.map((o) => [
      o.stt, o.cust, o.sdt,
      o.addr.startsWith("http") ? "[Ảnh: " + o.addr + "]" : o.addr,
  ...
  function exportExcelUnsent() {
    ...
    const rows = toExport.map((o) => [
      o.stt, o.cust, o.sdt,
      o.addr.startsWith("http") ? "[Ảnh địa chỉ: " + o.addr + "]" : o.addr,
