...
const Dashboard = () => {
  ...
  const [exchangeRate, setExchangeRate] = useState(6500);
  const [exchangeRateInput, setExchangeRateInput] = useState(6500);
  ...
  const fetchRecords = async () => {
    try {
      const response = await axios.get("/api/jp/recordsMKT");
  ...
  const fetchEmployees = async () => {
    try {
      ...
      setEmployees2(response.data.data);
    } catch (error) {
  ...
  const employeeChartDataNew = mktEmployees.map((emp) => {
    ...
    return { name: emp.name, profit: sales * 6500 , adsCost };
  });
  ...
  const employeeChartDataNewsale = saleEmployees2.map((emp) => {
    ...
    return { name: emp.name, profit: sales * 6500, fill: fillColor };
  });
  ...
  const teamChartDataNew2 = teams.map((team) => {
    ...
    return { name: team.label, profit: sales * 6500 , adsCost };
  });
  ...
  //     return acc + empAds;
  //   }, 0);
  //   return { name: team.label, profit: sales*150, adsCost };
  // });
  ...
  const teamChartDataNew = teams2.map((team) => {
    ...
    return {
      name: team.label,
      LeadAndMembers: sales * 6500,
      members: members * 6500,
    };
  });
  ...
  if (isFilterApplied && filteredOrders.length > 0) {
    ...
    dailyChartDataNew = dateArray.map((date) => {
      ...
      const adsCost = filteredAds
        ...
        .reduce((sum, ad) => sum + (ad.request1 + ad.request2), 0);
      return { name: date, profit: sales * 6500 , adsCost };
    });
  } else {
  ...
  const averageTeamChartData = teams2.map((team) => {
    ...
    return { name: team.label, profit: avgProfit * 6500  };
  });
  ...
  const leaderComparisonChartData = teams.map((team) => {
    ...
    const leaderSales =
      leaderSales0 !== 0
        ? ((adsCost / (leaderSales0 * 6500)) * 100).toFixed(2)
        : 0;
    const othersSales =
      othersSales0 !== 0
        ? ((adsCost2 / (othersSales0 * 6500)) * 100).toFixed(2)
        : 0;
...
const top5Employees2 = randomEmployee ? [randomEmployee] : [];
  ...
  const saleReportDataXL = saleEmployeesND.map((emp, index) => {
    let paid = 0,
      unpaid = 0;
    if (emp.position === "salenhapdon" || emp.position === "salefull") {
      paid = filteredOrders
  ...
  const dailySaleColumns = [
    ...
    },
    {
      ...
      dataIndex: "total",
      key: "total",
      render: (value) => (value * 6500).toLocaleString(),
    },
    {
      title: "SL Đơn",
      dataIndex: "sodon",
...
const summaryData = [
  {
    key: "1",
    today: (totalTodayProfit * 6500).toLocaleString("vi-VN") + " VNĐ",
    yesterday: (totalYesterdayProfit * 6500).toLocaleString("vi-VN") + " VNĐ",
        ...
        <div className="marquee">
          {top5Employees.map((emp, index) => (
            ...
            >
              ...
              <br />
              {emp.totalToday * 6500  > 15000000 && (
                <span className="employee-name2">
                  {(emp.totalToday * 6500 ).toLocaleString()} VNĐ
                </span>
              )}
            </div>
          ))}
