...
const Dashboard = () => {
  ...
  const [exchangeRateInput, setExchangeRateInput] = useState(150);
  ...
  const employeeChartDataNew = mktEmployees.map((emp) => {
    ...
    return { name: emp.name, profit: sales * 150 * 0.95, adsCost };
  });
  ...
  const employeeChartDataNewsale = saleEmployees2.map((emp) => {
    ...
    return { name: emp.name, profit: sales * 150, fill: fillColor };
  });
  ...
  const teamChartDataNew2 = teams.map((team) => {
    ...
    return { name: team.label, profit: sales * 150 * 0.95, adsCost };
  });
  ...
  if (isFilterApplied && filteredOrders.length > 0) {
    ...
    dailyChartDataNew = dateArray.map((date) => {
      ...
      const adsCost = filteredAds
        ...
        .reduce((sum, ad) => sum + (ad.request1 + ad.request2), 0);
      return { name: date, profit: sales * 150 * 0.95, adsCost };
    });
  } else {
  ...
  const averageTeamChartData = teams2.map((team) => {
    ...
    return { name: team.label, profit: avgProfit * 150 * 0.95 };
  });
  ...
  const leaderComparisonChartData = teams.map((team) => {
    ...
    const leaderSales =
      leaderSales0 !== 0
        ? ((adsCost / (leaderSales0 * 150)) * 100).toFixed(2)
        : 0;
    const othersSales =
      othersSales0 !== 0
        ? ((adsCost2 / (othersSales0 * 150)) * 100).toFixed(2)
        : 0;
...
const top5Employees2 = randomEmployee ? [randomEmployee] : [];
  ...
  const dailySaleColumns = [
    ...
    },
    {
      ...
      dataIndex: "total",
      key: "total",
      render: (value) => (value * 150).toLocaleString(),
    },
    {
      title: "SL Đơn",
      dataIndex: "sodon",
...
const summaryData = [
  {
    key: "1",
    today: (totalTodayProfit * 150).toLocaleString("vi-VN") + " VNĐ",
    yesterday: (totalYesterdayProfit * 150).toLocaleString("vi-VN") + " VNĐ",
        ...
        <div className="marquee">
          {top5Employees.map((emp, index) => (
            ...
            >
              ...
              <br />
              {emp.totalToday * 150 * 0.95 > 15000000 && (
                <span className="employee-name2">
                  {(emp.totalToday * 150 * 0.95).toLocaleString()} VNĐ
                </span>
              )}
            </div>
          ))}
