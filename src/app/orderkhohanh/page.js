...
.vb-chuachuyenkho{background:var(--amber-s);color:var(--amber)}
.vb-dachuyenkho{background:var(--green-s);color:var(--green)}
...
.ni.v-chuachuyenkho:not(.on) .pill{background:#3a2800;color:#fbbf24}
.ni.v-dachuyenkho:not(.on) .pill{background:#0d2e1e;color:#34d399}
...
function mapOrder(o) {
  ...
  const delivered =
    o.deliveryStatus === "GIAO THÀNH CÔNG" && o.istick5 !== true;
  ...
  return {
    ...
    istick5: o.istick5 || false,
    istickDate: o.istickDate || null,
...
const VIEW_CFG = {
  ...
  chuachuyenkho: {
    title: "Chưa chuyển kho",
    sub: "Chưa có KHOTQ1",
    badgeCls: "vb-chuachuyenkho",
    badge: "Chưa chuyển",
  },
  dachuyenkho: {
    title: "Đã chuyển kho",
    sub: "Đã có KHOTQ1",
    badgeCls: "vb-dachuyenkho",
    badge: "Đã chuyển",
  },
  unsent: {
    title: "Chưa gửi hàng",
  ...
  switch (view) {
    ...
    case "chuachuyenkho":
      return orders.filter(
        (o) => !o.isShippingName || o.isShippingName !== "KHOTQ1",
      );
    case "dachuyenkho":
      return orders.filter((o) => o.isShippingName === "KHOTQ1");
    case "unsent":
      return orders.filter((o) => !o.ngayGui && o.isShippingName === "KHOTQ1");
    case "sent":
      return orders.filter(
        (o) =>
          o.ngayGui &&
          ...
          !o.reconciled &&
          !o.istick5 === true &&
          o.isShippingName === "KHOTQ1",
      );
    case "late":
      return orders.filter((o) => isLate(o) && o.isShippingName === "KHOTQ1");
    case "needprocess":
      return orders.filter(
        (o) =>
          o.istick5 === true &&
    ...
    case "processed":
      return orders.filter(
        (o) => o.daXuLy === true && o.isShippingName === "KHOTQ1",
      );
    case "done":
      return orders.filter(
        (o) => o.delivered && !o.reconciled && o.isShippingName === "KHOTQ1",
      );
    case "reconcile":
      return orders.filter(
        (o) => o.reconciled && o.isShippingName === "KHOTQ1",
      );
    default:
      return [];
  }
...
export default function KhoOrderList() {
  ...
  const fetchOrders = useCallback(async () => {
    setLoading(true);
    try {
      const res = await axios.get("/api/orders/orderkhohqhanh", {
        params: {
          filter: "failed",
  ...
  const handleNeedProcessNoteChange = (orderId, value) => {
    setNeedProcessForm((prev) => ({
      ...prev,
      [orderId]: {
        ...(prev[orderId] || {}),
  ...
  const handleNeedProcessToggle = (orderId, checked) => {
    setNeedProcessForm((prev) => ({
      ...prev,
      [orderId]: {
        ...(prev[orderId] || {}),
        checked,
        lyDo: checked
          ? prev[orderId]?.lyDo || `[${dayjs().format("DD/MM/YYYY HH:mm")}] `
          : prev[orderId]?.lyDo || "",
      },
    }));
  };

  const saveNeedProcessForm = async () => {
    const payload = Object.entries(needProcessForm)
      .filter(([, v]) => {
        const lyDo = (v?.lyDo || "").toString();
        if (!lyDo.trim()) return false;
        return lyDo !== (v?.savedLyDo || "");
      })
      .map(([id, v]) => ({
        id,
        istickLyDo: v.lyDo.toString().trim(),
      }));
    if (!payload.length) {
      showToast("Chưa có đơn cần lưu");
      return;
    }
    try {
      const { data } = await axios.post("/api/orders/updateIstickHistory", {
        orders: payload,
      });
      showToast(data.message || "Đã lưu đơn cần xử lý");
      fetchOrders();
    } catch (err) {
      console.error(err);
      showToast(err?.response?.data?.error || "Lỗi khi lưu");
    }
  };
  ...
  const pillCounts = useMemo(
    () => ({
      ...
      chuachuyenkho: ordersWithState.filter(
        (o) => o.isShippingName === "KHOTQ1",
      ).length,
      dachuyenkho: ordersWithState.filter(
        (o) => !o.ngayGui && o.isShippingName === "KHOTQ1",
      ).length,
      ...
      late: ordersWithState.filter(
        (o) => isLate(o) && o.isShippingName === "KHOTQ1",
      ).length,
      ...
      processed: ordersWithState.filter(
        (o) => o.daXuLy === true && o.isShippingName === "KHOTQ1",
      ).length,
      done: ordersWithState.filter(
        (o) => o.delivered && !o.reconciled && o.isShippingName === "KHOTQ1",
      ).length,
      reconcile: ordersWithState.filter(
        (o) => o.reconciled && o.isShippingName === "KHOTQ1",
      ).length,
    }),
    [ordersWithState],
  ...
  async function saveDelivered() {
    ...
    try {
      ...
      fetchOrders();
      // Nếu đang ở mục Đơn cần xử lý thì ở lại mục đó (vì các đơn đã tick Giao TC sẽ tự động biến mất khỏi đây do istick5 về false)
      if (currentView === "needprocess") {
        setTimeout(() => setCurrentView("needprocess"), 700);
      } else {
  ...
  // ── Chuyển kho (cập nhật isShippingName = KHOTQ1) ──
  async function chuyenKhoOrders() {
    const toUpdate = filteredList.filter(
      (o) =>
        selectedIds.has(o.id) &&
        (!o.isShippingName || o.isShippingName !== "KHOTQ1"),
    );

    if (!toUpdate.length) {
      showToast("Chưa chọn đơn nào hoặc đơn đã chuyển kho");
      return;
    }

    try {
      await axios.post("/api/orders/chuyen-kho", {
        orders: toUpdate.map((o) => ({
          stt: o.stt,
        })),
      });

      showToast(`Đã chuyển kho ${toUpdate.length} đơn ✓`);
      clearSelection();
      fetchOrders();
    } catch (err) {
      console.error(err);
      showToast("Lỗi khi chuyển kho");
    }
  }

  // ── Chuyển kho tất cả (trong view Chưa chuyển kho) ──
  async function chuyenKhoAll() {
    const toUpdate = filteredList.filter(
      (o) => !o.isShippingName || o.isShippingName !== "KHOTQ1",
    );

    if (!toUpdate.length) {
      showToast("Không có đơn nào để chuyển kho");
      return;
    }
  ...
  async function tickAllDelivered() {
    ...
    try {
      ...
      // Nếu đang ở mục Đơn cần xử lý thì các đơn vừa tick sẽ tự động biến mất (istick5 = false)
      // → quay lại "done" để xem kết quả
      setTimeout(() => {
        setCurrentView(currentView === "needprocess" ? "done" : "done");
      }, 700);
    } catch (err) {
      console.error(err);
  ...
  async function bulkDeliver() {
    ...
    try {
      ...
      // Nếu ở mục Đơn cần xử lý, các đơn đã tick sẽ tự biến mất → giữ nguyên view
      if (currentView !== "needprocess") {
        setTimeout(() => setCurrentView("done"), 700);
      }
    } catch (err) {
      ...
      <div className="kho-root">
        ...
        <aside className={`sb${sidebarOpen ? " open" : ""}`}>
          ...
          <div className="sbf">
            ...
            <div className="ver">v5.0 · KHOH</div>
          </div>
        </aside>
            ...
            <div className="tcard">
              <div className="ch">
                ...
                <div className="btn-row">
                  ...
                  {currentView === "chuachuyenkho" && (
                    <>
                      <button
                        className="btn btn-amber btn-sm"
                        onClick={() => setCurrentView("dachuyenkho")}
                      >
                        <svg
                          viewBox="0 0 24 24"
                          fill="none"
                  ...
                  {currentView === "dachuyenkho" && (
                    <>
                      <button
                        className="btn btn-amber btn-sm"
                        onClick={() => setCurrentView("chuachuyenkho")}
                      >
                        <svg
                          viewBox="0 0 24 24"
                          fill="none"
                  ...
                  <tbody>
                    {filteredList.length === 0 ? (
                      ...
                      filteredList.map((o) => {
                        ...
                        else if (o.istick5 === true)
                          dispSt = { cls: "needproc-c", label: "CẦN XỬ LÝ" };
                        else if (late)
                          dispSt = {
                            cls: "late-c",
                                ...
                                >
                                  ...
                                  >
                                    <textarea
                                      ...
                                      rows={1}
                                      value={
                                        needProcessForm[o.id]?.lyDo ??
                                        o.istickLyDo ??
                                        ""
                                      }
                                      onChange={(e) =>
                                        handleNeedProcessNoteChange(
                                          o.id,
                                          e.target.value,
                                        )
                                      }
                                      // onFocus={(e) => handleNeedProcessNoteFocus(o.id, e)}
                                      style={{
                                        flex: 1,
                                        minWidth: 120,
                                        resize: "vertical",
                                  ...
                                  {Array.isArray(o.istickHistory) &&
                                    o.istickHistory.length > 0 && (
                                      <div
                                        style={{
                                          fontSize: 10.5,
                                          color: "#555",
                                      ...
                                      >
                                        ...
                                        >
                                          Lịch sử ({o.istickHistory.length}):
                                        </div>
                                        {o.istickHistory.map((h, idx) => (
                                          <div
                                            key={idx}
              ...
              <div className="sb-bar">
                ...
                <div style={{ display: "flex", gap: 7, flexWrap: "wrap" }}>
                  ...
                  {currentView === "needprocess" &&
                    (() => {
                      const dirtyCount = Object.values(needProcessForm).filter(
                        (v) => {
                          const lyDo = (v?.lyDo || "").toString();
        ...
        <div className={`bulk-bar${selectedCount > 0 ? " show" : ""}`}>
          ...
          <div className="bulk-actions">
            ...
            {(currentView === "chuachuyenkho" || currentView === "all") && (
              <button className="bb bb-green" onClick={chuyenKhoOrders}>
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor">
                  <path d="M22 11.08V12a10 10 0 1 1-5.93-9.14" />
                  <path d="M22 4 12 14.01l-3-3" />
                </svg>
                Chuyển kho ({selectedCount})
              </button>
            )}
            {currentView === "done" && (
