...
const Dashboard = () => {
        ...
        return (
          <ResponsiveContainer width="100%" height={400}>
            <BarChart data={data}>
              ...
              />
              <Bar dataKey="profit" radius={[12, 12, 0, 0]} maxBarSize={48}>
                {data.map((entry, index) => (
                  <Cell
                    key={`cell-${index}`}
  ...
  const filteredOrdersByArea2 =
    selectedArea === "all"
      ...
      : filteredOrders.filter((order) => {
          ...
          return areaEmployeeNames.includes(mkt);
        });
  ...
  // Tạo dữ liệu cho PieChart dựa trên doanh số của từng team (MKT)
  const teamProfitData = teams2.map((team) => {
    const teamEmps = mktEmployeesWithSpecial.filter(
      (emp) => emp.team_id === team.value,
    );
    const totalProfit = teamEmps.reduce((sum, emp) => {
  ...
  const totalTeamSales = teamProfitData.reduce((sum, t) => sum + t.profit, 0);
  const employeePieData = teamProfitData.map((team) => ({
    ...team,
    percent:
      totalTeamSales > 0
        ? Number((team.profit / totalTeamSales) * 100).toFixed(2)
        : 0,
  }));

  const teamEmployees = mktEmployeesWithSpecial.filter(
    (emp) =>
  ...
  // Tạo danh sách đặc biệt cho team DIEN (không giới hạn quocgia)
  const specialTeamMembers = filteredEmployeesByArea.filter((emp) => {
    if (
      currentUser.team_id === "SON" &&
  ...
  // Lọc nhân viên MKT thuộc team (kết hợp với danh sách đặc biệt)
  const teamMktEmployees = mktEmployeesWithSpecial
    .filter((emp) => emp.team_id === currentUser.team_id)
    .concat(specialTeamMembers);
  ...
  const teamFilteredAds = filteredAdsByArea.filter((ad) =>
    teamEmployeeNames.includes(ad.name.trim().toLowerCase()),
  );

  // Xác định Leader trong team (dùng filter thay vì find để lấy tất cả leader)
  const teamLeaders = teamEmployees.filter(
    (emp) => emp.position === "lead" || emp.name === currentUser.name,
  );
  const leaderNames = teamLeaders.map(emp => emp.name.toLowerCase());
  ...
  const marketingReportDataTEAM = teamMktEmployees.map((emp, index) => {
    ...
    const totalAds = teamFilteredAds
      .filter((ad) => ad.name.trim().toLowerCase() === empNameLower)
      .reduce((sum, ad) => sum + (ad.request1 + ad.request2), 0);
  ...
  const renderPill = (percent, kind = "percent") => {
    ...
    return (
      <div className={`sale-pill sale-pill-${level}`}>{v.toFixed(2)}%</div>
    );
  };
  ...
  const saleColumns = [
    ...
    },
    {
      ...
      align: "right",
      render: (value) => (
        <span className="sale-num sale-num-paid">{fmtNum(value)}</span>
      ),
    },
    {
      title: "Chưa thanh toán",
      ...
      align: "right",
      render: (value) => (
        <span className="sale-num sale-num-unpaid">{fmtNum(value)}</span>
      ),
    },
    {
      title: "Tổng",
    ...
    },
    {
      ...
      align: "right",
      render: (value) => (
        <span className="sale-num sale-num-vnd">{fmtNum(value)}</span>
      ),
    },
    {
      title: "% đòi tiền",
                ...
                <div key={index} className={`lb-card ${ranks[index] || ""}`}>
                  ...
                  {showDs && (
                    <div className="lb-stat">
                      <strong>
                        {(emp.totalToday * 17000).toLocaleString("vi-VN")} VNĐ
                      </strong>
                      DS hôm nay
                    </div>
                  )}
      ...
      <div className="filter-bar-container">
        <div className="lfbar">
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <span
              style={{ fontSize: 12, fontWeight: 500, color: "var(--sub)" }}
            >
              Ngày:
            </span>
            <input
        ...
        >
          <div className="kpi-card green">
            <div className="kpi-lbl">
              <span>📊</span> Doanh số — {selectedTeam || "Tất cả"}
            </div>
            <div className="kpi-val">
              {(tongKW3 * exchangeRate).toLocaleString()} đ
            </div>
            <div className="kpi-sub">
              <span className="up">▲ +12.4%</span> so với trước
            </div>
          </div>
          <div className="kpi-card gold">
            <div className="kpi-lbl">
              <span>💰</span> Chi phí Ads — {selectedTeam || "Tất cả"}
            </div>
            <div className="kpi-val gold">{totalAdsKW3.toLocaleString()} đ</div>
            <div className="kpi-sub">
              <span className="dn">▼ Cá nhân</span>: {selectedTeam}
            </div>
          </div>
              ...
              >
                ...
                <div className="all-team-card">
                  <div className="all-team-card-header orange">
                    <div className="card-icon">📦</div>
                    <h3>
                      Thống kê đơn hàng (Tổng đơn / DONE / Đã gửi / Chưa gửi)
                    </h3>
                  </div>
                  <div className="all-team-card-body">
                    {(currentUser.position === "admin" ||
                            ...
                            return (
                              ...
                              >
                                ...
                                <td style={tdStyle}>
                                  ...
                                  >
                                    {Number(row.tienVND).toLocaleString()}{" "}
                                    <span style={{ fontWeight: "400" }}>đ</span>
                                  </span>
                                </td>
                                <td style={tdStyle}>
                                  ...
                                  >
                                    {Number(row.totalAds).toLocaleString()}{" "}
                                    <span style={{ fontWeight: "400" }}>đ</span>
                                  </span>
                                </td>
                        ...
                        <div className="sale-table-card">
                          <div className="sale-table-header">
                            ...
                            <div className="sale-table-header-text">
                              ...
                              <p className="sale-table-subtitle">
                                Doanh số và tỷ lệ đòi tiền nhân viên Xử lý vận
                                đơn
                              </p>
                            </div>
                          </div>
                      ...
                      {marketingReportDataTEAM.map((row, idx) => {
                        ...
                        return (
                          <tr
                            key={row.key}
                            onMouseEnter={(e) => {
                              e.currentTarget.style.backgroundColor = "#fff3a3";
                            }}
                            onMouseLeave={(e) => {
                          ...
                          >
                            ...
                            <td style={tdStyle}>
                              <span
                                style={{ color: "#722ed1", fontWeight: "700" }}
                              >
