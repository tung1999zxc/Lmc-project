...
const [selectedMKT, setSelectedMKT] = useState(null);
  ...
  const [selectedTeam, setSelectedTeam] = useState(isAdminUser ? "" : reduxCurrentUser.team_id);
          ...
          <ResponsiveContainer width="100%" height={isSaleShift ? 340 : 520}>
            <PieChart>
              ...
              <Legend
                ...
                wrapperStyle={{ fontSize: 13, fontWeight: 700, color: "#334155" }}
              />
            </PieChart>
          </ResponsiveContainer>
        );
...
const filteredAdsByArea2 = filteredAds.filter((ad) => {
  ...
  const employeeChartDataNew = useMemo(() => {
    ...
    for (let i = 0; i < filteredAdsByArea.length; i++) {
      ...
      if (!key) continue;
      adsByName.set(key, (adsByName.get(key) || 0) + ((a.request1 || 0) + (a.request2 || 0)));
    }
  ...
  const teamChartDataNew = useMemo(() => {
    ...
    return teams2.map((team) => {
      ...
      let sales = 0;
      for (let i = 0; i < teamEmps.length; i++) sales += lookup(teamEmps[i].name);
      let members = 0;
      for (let i = 0; i < teamEmps2.length; i++) members += lookup(teamEmps2[i].name);
...
const teamChartDataNew2 = useMemo(() => {
    ...
    const lookupAds = (name) =>
      adsByName.get(name.trim().toLowerCase()) || 0;
  ...
  }, [teams, filteredEmployeesByArea, filteredOrdersByArea, filteredAdsByArea2]);
  ...
  const dailyChartDataNew = useMemo(() => {
    ...
    for (let i = 0; i < salesSource.length; i++) {
      ...
      if (!d) continue;
      profitByDate.set(d, (profitByDate.get(d) || 0) + (salesSource[i].profit || 0));
    }
    ...
    for (let i = 0; i < adsSource.length; i++) {
      ...
      adsByDate.set(
        d,
        (adsByDate.get(d) || 0) + ((adsSource[i].request1 || 0) + (adsSource[i].request2 || 0))
      );
    }
    ...
    return result;
  }, [isFilterApplied, filteredOrders, filteredOrdersByArea, orders, filteredAdsByArea2, adsMoneyData]);
  ...
  const averageTeamChartData = useMemo(() => {
    ...
    return teams2.map((team) => {
      ...
      let teamProfit = 0;
      for (let i = 0; i < teamEmps.length; i++) teamProfit += lookup(teamEmps[i].name);
...
const top5Employees = marketingReportData3
  ...
  const marketingReportDataTEAM = teamMktEmployees.map((emp, index) => {
    ...
    return {
      ...
      totalAds,
      adsPercent,
      isLeader: ((emp.position || "").toLowerCase() === "lead") || ((emp.position || "").toLowerCase() === "managermkt"),
    };
  });
  ...
  const marketingColumns = [
    {
      ...
      fixed: "left",
      render: (text, record) => {
        const style = record && record.isLeader
          ? {
  ...
  const buildSaleReportData = (employeesList) => {
    ...
    for (let i = 0; i < filteredOrdersByArea.length; i++) {
      ...
      if (status === "ĐÃ THANH TOÁN") {
        ...
        if (saleKey) unpaidSale.set(saleKey, (unpaidSale.get(saleKey) || 0) + p);
  ...
  const saleColumns = [
    {
      ...
      render: (_, __, index) => {
        if (index < 3) {
          const colors = [
            { color: "#92400e", bg: "linear-gradient(145deg, #fef3c7, #fde68a)", border: "#fcd34d" },
            { color: "#475569", bg: "linear-gradient(145deg, #f8fafc, #e2e8f0)", border: "#cbd5e1" },
            { color: "#9a3412", bg: "linear-gradient(145deg, #ffedd5, #fed7aa)", border: "#fdba74" },
          ];
        ...
        return <span style={{ color: "#94a3b8", fontSize: 12 }}>{index + 1}</span>;
      },
    },
    {
      title: "Tên",
  ...
  const saleDailyData = useMemo(() => {
    ...
    for (let i = 0; i < filteredOrders.length; i++) {
      ...
      if (emp) {
        ...
        else if (emp.position_team2 === "hanhchinh") entry.hanhChinh += o.profit || 0;
        ...
        <div className="sale-report-grid">
          <section className="sale-panel sale-panel--daily card-sale-daily">
            <div className="sale-panel-heading">
              ...
              <span className="sale-panel-count">{saleDailyData.length} ngày</span>
            </div>
            <Table
          ...
          <section className="sale-panel sale-panel--shift card-sale-shift">
            ...
            <div className="sale-shift-summary">
              {shiftSummary.map((shift) => (
                <div className={`sale-shift-item sale-shift-item--${shift.tone}`} key={shift.label}>
        ...
        <div className="sale-employee-grid">
          <section className="sale-panel sale-panel--employee card-sale-employee">
            <div className="sale-panel-heading">
              ...
              <span className="sale-panel-count">{saleReportDataXL.length} nhân viên</span>
            </div>
            <Table
          ...
          <section className="sale-panel sale-panel--employee card-sale-employee-ol">
            <div className="sale-panel-heading">
              ...
              <span className="sale-panel-count">{saleReportDataOL.length} nhân viên</span>
            </div>
            <Table
          ...
          <section className="sale-panel sale-panel--employee card-sale-employee-nd">
            <div className="sale-panel-heading">
              ...
              <span className="sale-panel-count">{saleReportDataND.length} nhân viên</span>
            </div>
            <Table
              className="sale-report-table sale-report-table--compact"
              columns={saleColumnsOLND}
  ...
  return (
    ...
    >
      ...
      <div className={selectedTeam === "" ? "top-row" : ""}>
        ...
        <div className={`lb-wrap ${selectedTeam === "" ? "lb-wrap-all" : "lb-wrap-sm"}`}>
        ...
        {top5Employees.length === 0 ? (
          // === Loading state ===
          <div className="lb-list" style={{ justifyContent: 'center', alignItems: 'center', minHeight: 120 }}>
            <div style={{ color: 'rgba(255,255,255,0.7)', fontSize: 14, textAlign: 'center' }}>Đang tải dữ liệu...</div>
          </div>
        ) : selectedTeam === "" ? (
              ...
              return (
                <div key={index} className={`lb-row ${ranks[index]}`}>
                  ...
                  <div className="lb-row-info">
                    ...
                    <div className={`lb-row-tag ${ranks[index]}`}>{tops[index]}</div>
                  </div>
                  <div className="lb-row-count">
                    <div className={`lb-row-count-num ${ranks[index]}`}>{emp.orderCountToday || 0}</div>
              ...
              <div key={index} className={`lb-card ${ranks[index] || ''}`}>
                ...
                {showDs && (
                  <div className="lb-stat">
                    <strong>{(emp.totalToday * 1 * 0.95).toLocaleString('vi-VN')} VNĐ</strong>
                    DS hôm nay
                  </div>
                )}
              </div>
          ...
          <div className="lb-top10-body">
            ...
            {top10RevenueEmployees.length === 0 ? (
              ...
              top10RevenueEmployees.map((emp, idx) => {
                ...
                return (
                  <div key={emp.name} className={`lb-top10-row ${idx < 3 ? "lb-top10-row-podium" : ""}`}>
                    <div className="lb-top10-rank">{medal || `${idx + 1}`}</div>
                    <div className="lb-top10-name">{emp.name}</div>
                    <div className="lb-top10-team">{emp.team_name || emp.position_team || ""}</div>
                    <div className="lb-top10-ds">{ds.toLocaleString("vi-VN")} đ</div>
        ...
        <div className="team-stats">
          <div className="team-stat-card">
            <div className="team-stat-left">
              <img src="https://flagcdn.com/w80/kr.png" alt="KR" className="team-stat-flag" />
            ...
            <div className="team-stat-cells">
              <div className="ts-cell">
                ...
                <div className="ts-value ts-blue">{summaryKR[0].today.replace(" VNĐ", "")} <span className="ts-cur">VNĐ</span></div>
              </div>
              <div className="ts-divider-v"></div>
              <div className="ts-cell">
                <div className="ts-label">HÔM QUA</div>
                <div className="ts-value ts-green">{summaryKR[0].yesterday.replace(" VNĐ", "")} <span className="ts-cur">VNĐ</span></div>
              </div>
              <div className="ts-divider-v"></div>
          ...
          <div className="team-stat-card">
            <div className="team-stat-left">
              <img src="https://flagcdn.com/w80/my.png" alt="MY" className="team-stat-flag" />
            ...
            <div className="team-stat-cells">
              <div className="ts-cell">
                <div className="ts-label">HÔM NAY</div>
                <div className="ts-value ts-blue">{summaryJP[0].today.replace(" VNĐ", "")} <span className="ts-cur">VNĐ</span></div>
              </div>
              <div className="ts-divider-v"></div>
              <div className="ts-cell">
                <div className="ts-label">HÔM QUA</div>
                <div className="ts-value ts-green">{summaryJP[0].yesterday.replace(" VNĐ", "")} <span className="ts-cur">VNĐ</span></div>
              </div>
              <div className="ts-divider-v"></div>
              <div className="ts-cell ts-cell-pct">
                {(() => {
          ...
          <div className="team-stat-card">
            <div className="team-stat-left">
              <img src="https://flagcdn.com/w80/tw.png" alt="TW" className="team-stat-flag" />
            ...
            <div className="team-stat-cells">
              <div className="ts-cell">
                <div className="ts-label">HÔM NAY</div>
                <div className="ts-value ts-blue">{summaryTW[0].today.replace(" VNĐ", "")} <span className="ts-cur">VNĐ</span></div>
              </div>
              <div className="ts-divider-v"></div>
              <div className="ts-cell">
                <div className="ts-label">HÔM QUA</div>
                <div className="ts-value ts-green">{summaryTW[0].yesterday.replace(" VNĐ", "")} <span className="ts-cur">VNĐ</span></div>
              </div>
              <div className="ts-divider-v"></div>
              <div className="ts-cell ts-cell-pct">
                {(() => {
      ...
      {(currentUser.position === "lead" ||
        ...
        <Row gutter={[16, 16]}>
          <Col xs={24}>
            <div className="filter-bar-container">
              <div className="lfbar" style={{ flexWrap: "wrap", justifyContent: "space-between", rowGap: 12 }}>
                {/* Khu vực */}
        ...
        <Row>
          <Col xs={24}>
            <div className="kpi-strip" style={{ gridTemplateColumns: 'repeat(3, 1fr)', marginBottom: 16 }}>
              <div className="kpi-card-grad grad-gold">
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div className="kpi-lbl" style={{ marginBottom: 0 }}><span>💰</span> Tổng chi phí ads</div>
                  <div className="kpi-icon-box">💰</div>
                </div>
                <div className="kpi-val">
                  {Math.round(Number(totalAdsKW3 || 0)).toLocaleString('vi-VN')}
                  <span className="kpi-unit">đ</span>
                </div>
                <div className="kpi-sub">Marketing — {selectedTeam || 'Tất cả'}</div>
              </div>
              <div className="kpi-card-grad grad-green">
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div className="kpi-lbl" style={{ marginBottom: 0 }}><span>📊</span> Tổng</div>
                  <div className="kpi-icon-box">📊</div>
                </div>
                <div className="kpi-val">
                ...
                <div className="kpi-sub"><span className="up">▲</span> Doanh số — {selectedTeam || 'Tất cả'}</div>
              ...
              <div className="kpi-card-grad grad-blue">
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div className="kpi-lbl" style={{ marginBottom: 0 }}><span>📈</span> % chi phí ads</div>
                  <div
                    style={{
                      position: 'relative',
                      width: 52,
                ...
                <div className="kpi-val" style={{ marginTop: 4 }}>{Number(percentAds3 || 0).toFixed(1)}%</div>
                <div className="kpi-sub">
                  <span className="kpi-badge">
                    {Number(percentAds3 || 0) > 45 ? 'CẢNH BÁO' : Number(percentAds3 || 0) > 30 ? 'TRUNG BÌNH' : 'TỐT'}
                  </span>
                  Trên doanh số
                </div>
              </div>
      ...
      {((currentUser.position === "admin" && !selectedTeam) ||
        ...
        <>
          ...
          <Row>
            ...
            {selectedMKT ? (
  <>
    <h3 style={{ marginBottom: 16, fontSize: 18, fontWeight: 700, color: "#1e293b" }}>
      📊 {selectedMKT}
    </h3>
    <style>{`
      .mkt-stat-card {
    ...
    <div style={{ 
      ...
      <div className="mkt-stat-card mkt-stat-card-blue">
        <div style={{ fontSize: 14, marginBottom: 10, fontWeight: 600 }} className="mkt-label">
          📅 Doanh số hôm nay
        </div>
        <div style={{ fontSize: 26, fontWeight: 800 }} className="mkt-value">
          {totalTodayMKT.toLocaleString("vi-VN")} <span style={{ fontSize: 14 }}>VNĐ</span>
        </div>
      </div>
      ...
      <div className="mkt-stat-card mkt-stat-card-green">
        ...
        <div style={{ fontSize: 26, fontWeight: 800 }} className="mkt-value">
          {totalYesterdayMKT.toLocaleString("vi-VN")} <span style={{ fontSize: 14 }}>VNĐ</span>
        </div>
      </div>
      
      {/* Hôm nay đạt (%) */}
      <div className={`mkt-stat-card ${Number(percentMKT) > 100 ? "mkt-stat-card-teal" : Number(percentMKT) >= 80 ? "mkt-stat-card-yellow" : "mkt-stat-card-red"}`}>
        <div style={{ fontSize: 14, marginBottom: 10, fontWeight: 600 }} className="mkt-label">
          🎯 Hôm nay đạt (%)
        </div>
        <div style={{ fontSize: 32, fontWeight: 800 }} className="mkt-value">
          {(() => {
        ...
        <div className="team-stat-cells">
          ...
          <div className="ts-cell ts-cell-pct">
            {(() => {
              ...
              <h2 style={{ marginTop: "2rem", marginBottom: 14, fontSize: 18, fontWeight: 800, color: "#0f172a", display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", width: 32, height: 32, borderRadius: 8, background: "linear-gradient(135deg,#fef3c7 0%,#fde68a 100%)", fontSize: 16 }}>💰</span>
                Tổng hợp tài chính
              </h2>
              <div className="fin-stats">
                ...
                <div className="fin-stat-card">
                  <div className="fin-stat-head">
                    <div className="fin-stat-icon" style={{ background: "linear-gradient(135deg,#f0fdf4 0%,#dcfce7 100%)" }}>💳</div>
                    <div>
                      <div className="fin-stat-title">Tổng khách thanh toán</div>
                      <div className="fin-stat-sub">Doanh thu · {selectedArea || "Tất cả"}</div>
                    </div>
                  </div>
                  <div className="fin-stat-rows">
                    <div className="fin-row">
                      <span className="fin-row-label">Đã thanh toán</span>
                      <span className="fin-row-value blue">{Math.round(totalData[0].daThanhToan).toLocaleString()} <span className="fin-row-cur">VNĐ</span></span>
                    </div>
                    <div className="fin-row">
                      <span className="fin-row-label">Chưa thanh toán</span>
                      <span className="fin-row-value red">{Math.round(totalData[0].chuaThanhToan).toLocaleString()} <span className="fin-row-cur">VNĐ</span></span>
                    </div>
                    <div className="fin-row">
                      <span className="fin-row-label">Tổng</span>
                      <span className="fin-row-value">{Math.round(totalData[0].tong).toLocaleString()} <span className="fin-row-cur">VNĐ</span></span>
                    </div>
                  </div>
                  <div className="fin-stat-foot">
                    <span className="fin-stat-foot-label">Thanh toán đạt</span>
                    <span className={`fin-stat-pill ${totalData[0].thanhToanDat > 80 ? "good" : totalData[0].thanhToanDat >= 50 ? "warn" : "bad"}`}>
                ...
                <div className="fin-stat-card">
                  <div className="fin-stat-head">
                    <div className="fin-stat-icon" style={{ background: "linear-gradient(135deg,#eff6ff 0%,#dbeafe 100%)" }}>🛒</div>
                    <div>
                      <div className="fin-stat-title">Doanh số (SALE)</div>
                      <div className="fin-stat-sub">Lợi nhuận · {selectedArea || "Tất cả"}</div>
                    </div>
                  </div>
                  <div className="fin-stat-rows">
                    <div className="fin-row">
                      <span className="fin-row-label">Đã thanh toán</span>
                      <span className="fin-row-value blue">{Math.round(totalDataSALE[0].daThanhToan).toLocaleString()} <span className="fin-row-cur">VNĐ</span></span>
                    </div>
                    <div className="fin-row">
                      <span className="fin-row-label">Chưa thanh toán</span>
                      <span className="fin-row-value red">{Math.round(totalDataSALE[0].chuaThanhToan).toLocaleString()} <span className="fin-row-cur">VNĐ</span></span>
                    </div>
                    <div className="fin-row">
                      <span className="fin-row-label">Tổng</span>
                      <span className="fin-row-value">{Math.round(totalDataSALE[0].tong).toLocaleString()} <span className="fin-row-cur">VNĐ</span></span>
                    </div>
                  </div>
                  <div className="fin-stat-foot">
                    <span className="fin-stat-foot-label">Thanh toán đạt</span>
                    <span className={`fin-stat-pill ${totalDataSALE[0].thanhToanDat > 80 ? "good" : totalDataSALE[0].thanhToanDat >= 50 ? "warn" : "bad"}`}>
                ...
                {(currentUser.position === "admin" ||
                  ...
                  <div className="fin-stat-card">
                    <div className="fin-stat-head">
                      <div className="fin-stat-icon" style={{ background: "linear-gradient(135deg,#fef3c7 0%,#fde68a 100%)" }}>📣</div>
                      <div>
                        <div className="fin-stat-title">Doanh số (MKT)</div>
                        <div className="fin-stat-sub">Marketing · {selectedArea || "Tất cả"}</div>
                      </div>
                    </div>
                    <div className="fin-stat-rows">
                      <div className="fin-row">
                        <span className="fin-row-label">Tổng doanh số</span>
                        <span className="fin-row-value blue">{Math.round(totalData4[0].tong).toLocaleString()} <span className="fin-row-cur">VNĐ</span></span>
                      </div>
                      <div className="fin-row">
                        <span className="fin-row-label">Tổng chi phí Ads</span>
                        <span className="fin-row-value">{Math.round(totalData4[0].totalAds).toLocaleString()} <span className="fin-row-cur">KRW</span></span>
                      </div>
                    </div>
                    <div className="fin-stat-foot">
                      <span className="fin-stat-foot-label">% Chi phí Ads</span>
                      <span className={`fin-stat-pill ${totalData4[0].percentAds < 30 ? "good" : totalData4[0].percentAds <= 35 ? "warn" : "bad"}`}>
                ...
                {(currentUser.position === "admin" ||
                  ...
                  <div className="fin-stat-card">
                    <div className="fin-stat-head">
                      <div className="fin-stat-icon" style={{ background: "linear-gradient(135deg,#f3e8ff 0%,#e9d5ff 100%)" }}>📦</div>
                      <div>
                        <div className="fin-stat-title">Thống kê đơn hàng</div>
                        <div className="fin-stat-sub">Vận đơn · {selectedArea || "Tất cả"}</div>
                      </div>
                    </div>
                    {shippingReport ? (
                      <>
                        <div className="fin-stat-rows">
                          <div className="fin-row">
                            ...
                            <span className="fin-row-value">{shippingReport.C.toLocaleString()}</span>
                          </div>
                          <div className="fin-row">
                            <span className="fin-row-label">Đơn DONE (A)</span>
                            <span className="fin-row-value blue">{shippingReport.A.toLocaleString()}</span>
                          </div>
                          <div className="fin-row">
                            <span className="fin-row-label">Đã gửi (B)</span>
                            <span className="fin-row-value green">{shippingReport.B.toLocaleString()}</span>
                          </div>
                          <div className="fin-row">
                            <span className="fin-row-label">Chưa gửi (A-B)</span>
                            <span className="fin-row-value red">{shippingReport.notSent.toLocaleString()}</span>
                          </div>
                        </div>
                        <div className="fin-stat-foot">
                          <div style={{ display: "flex", flexDirection: "column", gap: 8, width: "100%" }}>
                            <div style={{ display: "flex", gap: 8, width: "100%" }}>
                              <div style={{ flex: 1, background: "#dcfce7", borderRadius: 8, padding: "8px 12px", textAlign: "center" }}>
                                <div style={{ fontSize: 11, color: "#166534", marginBottom: 2 }}>% DONE</div>
                                <div style={{ fontSize: 16, fontWeight: 700, color: "#166534" }}>{shippingReport.percentDoneText}</div>
                              </div>
                              <div style={{ flex: 1, background: "#dbeafe", borderRadius: 8, padding: "8px 12px", textAlign: "center" }}>
                                <div style={{ fontSize: 11, color: "#1e40af", marginBottom: 2 }}>% ĐÃ GỬI</div>
                                <div style={{ fontSize: 16, fontWeight: 700, color: "#1e40af" }}>{shippingReport.percentSentText}</div>
                              </div>
                              <div style={{ flex: 1, background: "#fee2e2", borderRadius: 8, padding: "8px 12px", textAlign: "center" }}>
                                <div style={{ fontSize: 11, color: "#991b1b", marginBottom: 2 }}>% CHƯA GỬI</div>
                                <div style={{ fontSize: 16, fontWeight: 700, color: "#991b1b" }}>{shippingReport.percentNotSentText}</div>
                              </div>
                            </div>
                            <div style={{ display: "flex", gap: 8 }}>
                              <div style={{ flex: Number(shippingReport.percentDoneText) || 0, height: 8, background: "#22c55e", borderRadius: 4, minWidth: 4 }} />
                              <div style={{ flex: Number(shippingReport.percentSentText) || 0, height: 8, background: "#3b82f6", borderRadius: 4, minWidth: 4 }} />
                              <div style={{ flex: Number(shippingReport.percentNotSentText) || 0, height: 8, background: "#ef4444", borderRadius: 4, minWidth: 4 }} />
                            </div>
                          </div>
                        </div>
                      </>
                    ) : (
                      <div className="fin-stat-empty">Chưa có dữ liệu đơn hàng</div>
                    )}
                  </div>
                )}
                ...
                <div className="fin-stat-card">
                  <div className="fin-stat-head">
                    <div className="fin-stat-icon" style={{ background: "linear-gradient(135deg,#ffe4e6 0%,#fecaca 100%)" }}>⏰</div>
                    <div>
                      <div className="fin-stat-title">Thống kê giục CK</div>
                      <div className="fin-stat-sub">Đang chờ thanh toán · {selectedArea || "Tất cả"}</div>
                    </div>
                  </div>
                  <div className="fin-stat-rows">
                    {transferData.map((r, idx) => (
                      <div className="fin-row" key={idx}>
                        ...
                        <span className="fin-row-value green">{Number(r.daGuiHang).toLocaleString()}</span>
                      </div>
                    ))}
                    {transferData.map((r, idx) => (
                      <div className="fin-row" key={"c-" + idx}>
                        ...
                        <span className="fin-row-value red">{Number(r.chuaGuiHang).toLocaleString()}</span>
                      </div>
                    ))}
                    {transferData.map((r, idx) => (
                      <div className="fin-row" key={"t-" + idx}>
                        ...
                        <span className="fin-row-value">{Number(r.tong).toLocaleString()}</span>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          const mktTabContent = (
            <>
              ...
              <div className="kpi-strip" style={{ gridTemplateColumns: 'repeat(4, 1fr)', marginBottom: 16 }}>
                <div className="kpi-card-grad grad-green">
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div className="kpi-lbl" style={{ marginBottom: 0 }}><span>📊</span> Doanh số</div>
                    <div className="kpi-icon-box">📊</div>
                  </div>
                  <div className="kpi-val">
                    {Math.round(Number(tongKW3 || 0)).toLocaleString('vi-VN')}
                  ...
                  <div className="kpi-sub"><span className="up">▲</span> MKT — {selectedTeam || 'Tất cả'}</div>
                  <div className="kpi-progress">
                    <div className="kpi-progress-fill" style={{ width: '100%' }} />
                  </div>
                </div>
                ...
                <div className="kpi-card-grad grad-purple">
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div className="kpi-lbl" style={{ marginBottom: 0 }}><span>👥</span> DS / Ads NV</div>
                    <div className="kpi-icon-box">👥</div>
                  </div>
                  <div className="kpi-val" style={{ color: '#fff' }}>
                    {(() => {
                      const nvData = marketingReportData.filter(r => !r.isLeader);
                      const dsNV = nvData.reduce((sum, r) => sum + Number(r.tienVND || 0), 0);
                      const adsNV = nvData.reduce((sum, r) => sum + Number(r.totalAds || 0), 0);
                      return (
                        <div>
                          <div style={{ fontSize: '22px', fontWeight: 800, color: '#fff', lineHeight: 1.15 }}>
                            {Math.round(dsNV).toLocaleString('vi-VN')}
                            <span style={{ fontSize: '12px', fontWeight: 700, opacity: 0.75, marginLeft: 2 }}>đ</span>
                          </div>
                          <div className="kpi-sub">
                            <span>Ads:</span> {Math.round(adsNV).toLocaleString('vi-VN')}
                            <span style={{ fontSize: '10px', fontWeight: 700, opacity: 0.75, marginLeft: 1 }}>đ</span>
                          </div>
                        </div>
                      );
                    })()}
                  </div>
                  <div className="kpi-sub">{marketingReportData.filter(r => !r.isLeader).length} nhân viên</div>
                </div>
              </div>
              ...
              <Col xs={24} md={12}>
                <div className="card-mkt-table" style={{ padding: "16px", marginBottom: 16 }}>
                  ...
                  <Table
                    ...
                    rowClassName={(record) => (record.isLeader ? "leader-row" : "")}
                  />
                </div>
              </Col>
            </Row>
