...
const Dashboard = () => {
  ...
  const fetchOrders = async () => {
    ...
    const response = await axios.get(`/api/jp/ordersMKT?start=${start}&end=${end}`);
  ...
  const computeTotalSales = (employeeName) => {
    const totalProfit =
      safeOrders
        ...
        .reduce((sum, p) => sum + p.profit, 0) * 150;
  ...
  try {
    ...
    const start = dates[0];
    const end = dates[dates.length - 1];
    const response = await axios.get(`/api/jp/recordsMKT?start=${start}&end=${end}`);
    ...
    try {
      if (editingRecord) {
        const response = await axios.put(
          `/api/jp/recordsMKT/${editingRecord.id}`,
          newRecord
        );
        ...
        const response = await axios.post("/api/jp/recordsMKT", newRecord);
  ...
  const onSave = async (record) => {
    try {
      ...
      const response = await axios.put(`/api/jp/recordsMKT/${record.id}`, record);
  ...
  const adminSummaryData = summaryDates.map((date) => {
    ...
    const dsTong =
      safeOrders
        ...
        .reduce((sum, order) => sum + order.profit, 0) * 150;
  ...
  const onDelete = async (record) => {
    try {
      const response = await axios.delete(`/api/jp/recordsMKT/${record.id}`);
  ...
  const columns = [
    ...
    // },
    {
      ...
      render: (_, record) => {
        ...
        return (totalSalesForSelectedDate * 0.95 * 150).toLocaleString(
          "vi-VN"
        );
      },
    },
    {
      ...
      render: (_, record) => {
        ...
        const total = totalSalesForSelectedDate * 150 * 0.95;
