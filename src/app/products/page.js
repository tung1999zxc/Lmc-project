...
const InventoryPage = () => {
  ...
  const orders =
    selectedPreset === "all"
      ? orders2
      : filterByPreset(orders2, selectedPreset);
 ...
 const getTotalImportedQty = (product) => {
  ...
  importDate: values.importDate.format("YYYY-MM-DD"),
    ...
    try {
      ...
      const response = await axios.put(`/api/products/${productToUpdate.key}`, {
  ...
  const totalRevenue =
    searchText.trim() !== ""
      ? filteredProducts.reduce((acc, product) => {
          const productProfit = orders.reduce((sum, order) => {
            if (order.products && Array.isArray(order.products)) {
              if (
                order.products.some((item) => item.product === product.name)
              ) {
      ...
      : 0; // Nếu `searchText` rỗng, tổng doanh số sẽ là 0
          ...
          record.sltqHistory && record.sltqHistory.length > 0 ? (
            <ul style={{ padding: 0, margin: 0, listStyle: "none" }}>
              {record.sltqHistory.map((item, index) => (
                <li key={index}>
                  <strong>Ngày:</strong> {item.date} - <strong>SL:</strong>{" "}
                  {item.qty}
                </li>
              ))}
            </ul>
      ...
      >
        ...
        >
          ...
          <Option value="2currentMonth">
            2 Tháng (Từ đầu tháng đến hiện tại)
          </Option>
          <Option value="3currentMonth">
            3 Tháng (Từ đầu tháng đến hiện tại)
          </Option>
