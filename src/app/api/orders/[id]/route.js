...
export async function PUT(request, { params }) {
  try {
    ...
    delete data.istickLyDo;
    delete data.istickHistory;
    delete data.istickDate;

    const currentUser_kho =
      decodeURIComponent(request.headers.get("x-current-user")) ||
      data.updatedBy ||
      "Unknown";
    const allowedForKhoOnly = [
      "shippingDate1",
      "shippingDate2",
      "trackingCode",
      "noteKHO",
...
export async function DELETE(request, { params }) {
  try {
    ...
    // Chỉ cho phép từ trang orders
    if (
      origin !== "https://www.lmcgroupss.com" ||
      !referer?.startsWith("https://www.lmcgroupss.com/orders")
    ) {
      return new Response(
        JSON.stringify({ error: "Không được phép truy cập API này" }),
        { status: 403 },
      );
    }
