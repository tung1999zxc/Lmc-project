...
const Dashboard = () => {
  ...
  const fetchOrders = async () => {
    try {
      const response = await axios.get(
        `/api/orderssale?selectedPreset=${period}`,
      );
  ...
  const computeAverageClosingRate = (employeeName) => {
    ...
    employeeRecords.forEach((record) => {
      ...
      totalClosedOrders += computeTotalSalesNumberForDate(
        record.date,
        employeeName,
      );
    });
    const avgRate =
      totalNewMess === 0 ? 0 : (totalClosedOrders / totalNewMess) * 100;
    ...
    return (
      sampleOrders
        ...
        .reduce((acc, p) => {
          ...
          const scorePoints = Number(p.scorePoints ?? 100);
  ...
  const getDoneOrders = () => {
    return sampleOrders.filter(
      (p) => p.saleReport === "DONE" && filterSampleOrdersByPeriod(p),
    ).length;
  };
  ...
  const getTotalRemarketing = () => {
    return filteredRecords.reduce(
      (sum, r) => sum + (r.totalRemarketing || 0),
      0,
    );
  };
        ...
        >
          ...
          >
            <div>
              ...
              <div style={{ fontSize: 20, fontWeight: "bold" }}>
                {statsData.totalSales.toLocaleString("vi-VN")} ₫
              </div>
              <div style={{ fontSize: 12, opacity: 0.9, marginTop: 4 }}>
                {statsData.salesChange >= 0 ? "▲" : "▼"}{" "}
                {Math.abs(statsData.salesChange)}% so tháng trước
              </div>
            </div>
          ...
          >
            <div>
              ...
              <div style={{ fontSize: 20, fontWeight: "bold" }}>
                {statsData.closedOrders}
              </div>
              <div style={{ fontSize: 12, opacity: 0.9, marginTop: 4 }}>
                Tổng {statsData.totalOrders} đơn tháng này
              </div>
            </div>
            <div style={{ fontSize: 36 }}>✅</div>
          </div>
          ...
          >
            <div>
              ...
              <div style={{ fontSize: 20, fontWeight: "bold" }}>
                {statsData.avgChotRatio}%
              </div>
              <div style={{ fontSize: 12, opacity: 0.9, marginTop: 4 }}>
                Trung bình tỉ lệ chốt
              </div>
            </div>
            <div style={{ fontSize: 36 }}>📊</div>
          </div>
          ...
          >
            <div>
              <div style={{ fontSize: 12, opacity: 0.9, marginBottom: 4 }}>
                Mess mới/Mess tiếp thị
              </div>
              <div style={{ fontSize: 20, fontWeight: "bold" }}>
                {statsData.totalNewMess}/{statsData.totalRemarketing}
              </div>
              <div style={{ fontSize: 12, opacity: 0.9, marginTop: 4 }}>
                Tin nhắn xử lý tháng này
              </div>
            </div>
            <div style={{ fontSize: 36 }}>💬</div>
          </div>
          ...
          >
            ...
            >
              ...
              >
                ...
                <div
                  style={{ fontSize: 20, fontWeight: "bold", color: "#fa8c16" }}
                >
                  {statsData.processingOrders} đơn
                </div>
              </div>
              <div
                style={{
            ...
            >
              ...
              <Form.Item
                ...
                rules={[
                  {
                    required: true,
                    message: "Vui lòng nhập số mess tiếp thị lại",
                  },
                ]}
                style={{ marginBottom: 0 }}
              >
                <InputNumber
                  style={{ width: "100%" }}
                  placeholder="Tổng mess tiếp thị lại"
                />
              </Form.Item>
        ...
        >
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <span style={{ fontWeight: 600, color: "#333", minWidth: 120 }}>
                Khoảng thời gian:
              </span>
            </div>
            <Select
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <span style={{ fontSize: 18 }}>👥</span>
              <span style={{ fontWeight: 600, color: "#333", minWidth: 100 }}>
                Ca làm việc:
              </span>
            </div>
            <Select
          ...
          currentUser.position === "leadSALE" ? (
                    ...
                    >
                      ...
                      >
                        <div style={{ textAlign: "center" }}>
                          ...
                          >
                            {computeTotalSales(employeeName)} đ
                          </div>
                        </div>
                        <div
                          style={{
                        ...
                        <div style={{ textAlign: "center" }}>
                          ...
                          >
                            {computeAverageClosingRate(employeeName)}
                          </div>
                        </div>
                      </div>
                    </div>
            ...
            <div>
              ...
              >
                ...
                <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
                  <div style={{ textAlign: "center" }}>
                    ...
                    >
                      {computeTotalSales(currentUser.name)} đ
                    </div>
                  </div>
                  <div
                    style={{ width: 1, background: "rgba(255,255,255,0.3)" }}
                  />
                  <div style={{ textAlign: "center" }}>
                    <div
                      style={{ color: "rgba(255,255,255,0.7)", fontSize: 11 }}
                    ...
                    <div
                      style={{ color: "#fff", fontSize: 15, fontWeight: 700 }}
                    >
                      {computeAverageClosingRate(currentUser.name)}
                    </div>
                  </div>
                </div>
              </div>
              <Table
                dataSource={[
                  ...filteredRecords.filter(
                    (record) => record.employeeName === currentUser.name,
                  ),
                ].sort(
                  (a, b) => moment(b.date).valueOf() - moment(a.date).valueOf(),
                )}
