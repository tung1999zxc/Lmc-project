...
function buildAdminPoiQuery(road, jibun) {
  ...
  if (!primary) return '';
  const adminMatch = primary.match(/[\uAC00-\uD7A3]{2,}(?:\uC2DC|\uAD6C|\uAD70|\uC74D|\uBA74|\uB3D9)/g);
  const buildingMatch = primary.match(/[\uAC00-\uD7A3]{2,}(?:\uC544\uD30C\uD2B8|\uBE4C\uB77C|\uC624\uD53C\uC2A4\uD154|\uD0C0\uC6CC|\uD558\uC6B0\uC2A4|\uBE4C\uB529|\uC624\uD53C\uC2A4|\uB9E8\uC158|\uCE90\uC2A4|\uD329\uB9AC\uC2A4|\uD31C\uD06C|\uD558\uC774\uBE4C|\uB808\uC9C0\uB358\uC2A4|\uD78C\uC988|\uD478\uB974\uC9C0\uC624|\uB798\uBBF8\uC548|\uC790\uC774|\uB86F\uB370\uCE90\uC2A4|\uB354\uC0D8|\uC13C\uD2B8\uB808\uBE4C|\uC2A4\uCE74\uC774|\uC232)/g);
  if (!adminMatch && !buildingMatch) return '';
  const admin = adminMatch ? adminMatch.slice(0, 1).join(' ') : '';
  const bld = buildingMatch ? buildingMatch.slice(0, 2).join(' ') : '';
  let query = [admin, bld].filter(Boolean).join(' ').trim();
  if (!bld) {
    const lastHangul = primary.match(/[\uAC00-\uD7A3][\uAC00-\uD7A30-9]+/g);
    if (lastHangul && lastHangul.length > 0) {
      query = (admin ? admin + ' ' : '') + lastHangul.slice(-2).join(' ');
    }
  }
  return query;
}
