...
function cleanAddressNoise(input) {
  ...
  s = s.replace(/\s+\d+\uD638\b/g, ''); // bỏ "1302호" giữa câu
...
function buildAdminPoiQuery(road, jibun) {
  ...
  let finalBld = bld;
  if (aptInParen && /\uC8FC\uACF5|\uC544\uD30C\uD2B8|\uBE4C\uB77C|\uB9E8\uC158|\uD0C0\uC6CC|\uD78C\uC988/.test(aptInParen[1])) {
    finalBld = aptInParen[1].split(/\s+/).filter((t) => t.length >= 2).slice(-3).join(' ');
  }
  return [admin, finalBld].filter(Boolean).join(' ').trim();
}
